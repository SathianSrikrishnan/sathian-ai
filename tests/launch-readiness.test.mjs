...
test("parent-facing copy leads with first forever memory instead of bank language", () => {
  ...
  assert.match(homepage, /your child's first digital wallet/i)
  assert.match(homepage, /Toothlight/i)
  assert.match(homepage, /Parents control the wallet, the timing, and the family link/i)
  assert.match(homepage, /Time-locked until ready/i)
  assert.match(homepage, /A tiny ritual\./i)
  assert.match(homepage, /Then a wallet parents control/i)
  assert.match(homepage, /Built on Solana/i)
  assert.match(homepage, /href="#how-it-works"/)
  assert.match(homepage, /id="how-it-works"/)
  assert.match(homepage, /The first shelf/)
  assert.match(homepage, /Meet Tanda and the collectors of the Tooth Fairy Network/)
  ...
  assert.match(footer, /A tiny ritual\. A wallet parents control/)
...
test("about page keeps technology quiet and parent-first", () => {
  ...
  assert.match(about, /first forever memory/i)
  assert.match(about, /family/i)
  assert.match(about, /tribe/i)
  assert.match(about, /compounding/i)
...
test("stories atlas leads with worldbuilding and a subtle family-memory bridge", () => {
  ...
  assert.match(stories, /Story roadmap/)
  assert.match(stories, /The atlas opens the cultures behind the tooth/)
  assert.match(stories, /Ask someone who remembers/)
  assert.match(stories, /Make it a child-grandparent story activity/)
  assert.match(stories, /Add your story/)
  assert.match(stories, /collector roster/i)
