...
assert(/Add a note or gift/.test(source), 'family flow must combine note and gift as one family action')
assert(/A note is enough/.test(source), 'family flow must frame note as the default path')
assert(/Include a gift/.test(source), 'family flow must keep gift as an optional checkbox')
assert(/Add to Toothlight/.test(source), 'family primary CTA must add the family contribution')
assert(/View saved Toothlight/.test(source), 'family completion must offer a return to the saved Toothlight')
assert(!/Gift linked|Family gift linked|Family note \+ gift/.test(source), 'family flow must avoid fund-style linked language')
