...
assert(/Add the tooth/.test(editor), 'studio must lead with Add the tooth product language')
...
assert(/AI creates a story object from the memory and drawing/.test(client), 'make flow must explain the AI final as a story object')
...
assert(/Render AI final image/.test(client), 'make flow must expose optional AI-rendered final image action')
