...
assert(/Add photo/.test(editor), 'empty memory stage must use minimal image-first placeholder copy')
assert(!/Add the tooth photo, then draw what happened around it/.test(editor), 'empty memory stage must not repeat long instructional copy')
...
assert(/VoiceAssistField/.test(client), 'story step must use Voice Assist so the child can speak the memory')
assert(/aria-label="Child story"/.test(client), 'story step must be framed as the child story')
assert(/Tap mic\. Talk\. Save\./.test(client), 'story voice prompt must be short enough for thumb-first capture')
assert(/<h2>Tell it\.<\/h2>/.test(client), 'story step must use a short child-facing prompt')
assert(!/>Add the story</.test(client), 'story step must remove the redundant Add the story heading')
assert(!/>Child name</.test(client), 'story form must shorten the Child name label')
assert(!/>Toothlight name</.test(client), 'story form must shorten the Toothlight name label')
assert(!/>What happened\?</.test(client), 'story form must replace What happened with a voice-first memory field')
assert(!/After saving, seal the private note for later\. Smile Fund stays optional\./.test(client), 'save panel must remove explanatory copy after the story step')
assert(/<h2 className=\{styles\.srOnly\}>Save this Toothlight<\/h2>/.test(client), 'save panel heading must be accessible without adding visible copy')
assert(!/<h2>Save\.<\/h2>/.test(client), 'save panel must not add a redundant visible save heading')
