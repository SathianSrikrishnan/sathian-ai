...
- Phone voice/save follow-up on 2026-06-09 changed phones to use native browser speech first when available, fall back to recorded transcription, flush recorder data before stopping, and continue same-Wi-Fi local saves as development-only `local-*` Toothlights if a local save POST fails before returning a clean auth response.
...
- Focused mobile voice/save proof passed with `20 passed`, and the full mobile proof passed with `22 passed` across `Mobile Safari` and `Mobile Chrome`, covering local auth fallback, local failed-POST save fallback, native speech-first, speech-to-record fallback, recorder flush, mobile `audio/mp4` to `m4a`, blocked-mic recovery, make, note, saved Toothlight, family invite, and reveal preview.
